...
export const pqrService = {
  ...
  async getPQRs(
    ...
    const response = await axios.get(`/v1/pqr?${params.toString()}`);
    return response.data;
  },

  // Obtener métricas de PQRs
  async getPQRMetrics(): Promise<PQRMetrics> {
    const response = await axios.get('/v1/pqr/metrics');
    return response.data;
  },

  // Obtener una PQR específica
  async getPQR(id: string): Promise<PQR> {
    const response = await axios.get(`/v1/pqr/${id}`);
    return response.data;
  },

  // Crear nueva PQR
  async createPQR(data: PQRCreate): Promise<PQR> {
    const response = await axios.post('/v1/pqr', data);
    return response.data;
  },

  // Actualizar PQR
  async updatePQR(id: string, data: PQRUpdate): Promise<PQR> {
    const response = await axios.put(`/v1/pqr/${id}`, data);
    return response.data;
  },

  // Responder a una PQR
  async responderPQR(id: string, respuesta: string): Promise<PQR> {
    const response = await axios.post(`/v1/pqr/${id}/responder`, { respuesta });
    return response.data;
  },
  ...
  async cambiarEstado(
    ...
    const response = await axios.post(`/v1/pqr/${id}/cambiar-estado`, { nuevo_estado });
    return response.data;
  },
  ...
  async cambiarPrioridad(
    ...
    const response = await axios.post(`/v1/pqr/${id}/cambiar-prioridad`, { nueva_prioridad });
    return response.data;
  },

  // Eliminar PQR
  async deletePQR(id: string): Promise<void> {
    await axios.delete(`/v1/pqr/${id}`);
  },

  // Obtener PQRs de un cliente específico
  async getPQRsByCliente(clienteId: string): Promise<PQR[]> {
    const response = await axios.get(`/v1/pqr/cliente/${clienteId}`);
    return response.data;
  },
};